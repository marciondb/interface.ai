...
describe('fromCapabilityFile', () => {
  ...
  it.each<[string, (file: File & Record<string, unknown>) => void, string]>([
    ...
    ['an unknown target', (f) => { f.steps[1].action.target = 'detail.nope'; }, 'steps.1.action.target: unknown target detail.nope'],
    ['an unknown checkpoint target', (f) => { f.steps[0].checkpoint.target = 'lookup.nope'; }, 'steps.0.checkpoint.target: unknown target lookup.nope'],
    ['an unknown recover target', (f) => { f.outcomes[1].recover = { kind: 'click', target: 'nope' }; }, 'outcomes.1.recover.target: unknown target nope'],
    ['an undeclared placeholder', (f) => { f.steps[0].action.value = '{{inputs.nope}}'; }, 'steps.0.action.value: placeholder names undeclared input nope'],
    ['a malformed placeholder', (f) => { f.steps[0].action.value = '{{memberId}}'; }, 'steps.0.action.value: placeholders must look like {{inputs.<name>}}'],
    ['an output without a read', (f) => { f.steps.pop(); }, 'outputs.balance: output must be produced by exactly one read step, found 0'],
    ['an output read twice', (f) => { f.steps.push({ ...f.steps[1], id: 'read-again' }); }, 'outputs.balance: output must be produced by exactly one read step, found 2'],
    ['a read of an undeclared output', (f) => { f.steps[1].action.output = 'total'; }, 'steps.1.action.output: undeclared output total'],
    ['a duplicate step id', (f) => { f.steps[1].id = 'enter-member-id'; }, 'steps.1.id: duplicate step id enter-member-id'],
    ['a duplicate outcome id', (f) => { f.outcomes[1].id = 'member_not_found'; }, 'outcomes.1.id: duplicate outcome id member_not_found'],
    ['an unknown key', (f) => { Object.assign(f.steps[0], { retries: 3 }); }, 'steps.0: Unrecognized key: "retries"'],
    ...
    ['an invalid checkpoint pattern', (f) => { f.steps[1].checkpoint.pattern = '('; }, 'steps.1.checkpoint.pattern: pattern is not a valid regular expression'],
